...
在线段树上的每个节点上，除了区间端点外，再维护4个信息∶区间和 sum，区间最大连续子段和 dat，紧靠左端的最大连续子段和 lmax，紧靠右端的最大连续子段和rmax。
...
{
    int l, r;
    int lv, rv, v, sum;//lv靠近左端的最大连续子段值 rv靠近右端的最大连续子段值 v最大值 sum区间和
};
Node tr[4 * N];
...
void pushup(int u)
{
    ...
    tr[u].v = max({tr[2 * u].v, tr[2 * u + 1].v, tr[2 * u].rv + tr[2 * u + 1].lv});
    tr[u].lv = max(tr[2 * u].lv, tr[2 * u].sum +tr[2 * u + 1].lv);
    tr[u].rv = max(tr[2 * u + 1].rv, tr[2 * u + 1].sum + tr[2 * u].rv);
}
...
Node query(int u,int l,int r){
    if(tr[u].l >= l && tr[u].r <= r) return tr[u];//l<=tr[u].l && tr[u].r <=r
    int mid = (tr[u].l+tr[u].r)>>1;
    if(r <= mid) return query(2*u,l,r);//tr[u].l<=r && r<=mid
    if(l > mid) return query(2*u+1,l,r);
    Node ans,lnode,rnode;
    lnode = query(2*u,l,r);
    rnode = query(2*u+1,l,r);
  
    ans.sum = lnode.sum + rnode.sum;
    ans.v = max({lnode.v,rnode.v,lnode.rv+rnode.lv});
    ans.lv = max(lnode.lv,lnode.sum+rnode.lv);
    ans.rv = max(rnode.rv,rnode.sum+lnode.rv);
    return ans;
}
...
void modify(int u,int pos,int v){
    ...
    int mid = (tr[u].l+tr[u].r)>>1;
    if(pos <= mid) modify(2*u,pos,v);
    else modify(2*u+1,pos,v);
    pushup(u);
}
...
维护差分数组的前缀和 和 区间gcd 将区间修改变成单点修改
...
void pushup(int u)
{
    tr[u].v = abs(__gcd(tr[2 * u].v, tr[2 * u + 1].v));
    tr[u].sum = tr[2 * u].sum + tr[2 * u + 1].sum;
}
...
Node query(int u, int l, int r)
{
    ...
    lnode = query(2 * u, l, r);
    rnode = query(2 * u + 1, l, r);
    ans.v = abs(__gcd(lnode.v, rnode.v));
    ans.sum = lnode.sum + rnode.sum;
    return ans;
}
...
sum:如果考虑当前节点及子节点上的所有标记（没有所有祖先节点的标记），当前区间的和是多少。
...
void modify(int u,int l,int r,int d){
    ...
    if(root.l>=l && root.r <=r){
        root.add+=d;
        root.sum+=(ll)d*(root.r-root.l+1);
        return;
    }
    pushdown(u);
    int mid=(root.l+root.r)>>1;
    if(r<=mid) modify(2*u,l,r,d);
    else if(l>mid) modify(2*u+1,l,r,d);
    else{
        modify(2*u,l,mid,d);
        modify(2*u+1,mid+1,r,d);
    }
    pushup(u);
}
...
ll query(int u,int l,int r){
    ...
    else return query(2*u,l,mid)+query(2*u+1,mid+1,r);
}
...
将a[i]^a[i+1]^a[i+2]^.....^a[j]转化为s[i-1]^s[j]的异或。  
...
![image-20220324205252518](vscode-webview-resource://0c7t5uvccivgltr2premucs96o1bc8dja3hnmmavq844da3qqibf/file///C%3a/Users/jshmj/AppData/Roaming/Typora/typora-user-images/image-20220324205252518.png)
...
![image-20220519200854566](C:\Users\jshmj\AppData\Roaming\Typora\typora-user-images\image-20220519200854566.png)

思路 1-N的任务 先按照截止时间的顺序执行，如果遇到不能完成的任务，但是比之前完成过的某一个任务更优 那么就替换它。
