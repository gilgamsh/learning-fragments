...
//枚举i的所有真子集 for(int j=(i-1)&i;j!=0;j=(j-1)&i);
```
...
**push up操作** 用子节点信息算父节点信息



**push down操作** 父节点的修改信息 更新到子节点（懒标记，延迟标记）
...
set, map, multiset, multimap, 基于平衡二叉树（红黑树），动态维护有序序列
    ...
    set/multiset
        ...
        lower_bound()/upper_bound()
            lower_bound(x)  返回大于等于x的最小的数的迭代器 上确界
            upper_bound(x)  返回大于x的最小的数的迭代器 s
    map/multimap
        insert()  插入的数是一个pair
...
2. 归纳法：先算得出边界情况（例如n=1 ）的最优解F1 ，然后再证明：对于每个n+1， 都可以由 Fn推导出结果。
...
按区间**右端点** 从小到达排序   从前往后 枚举每一个区间  已经包含 pass 否则选取右端点 O(n)

证明 ans<=cnt  ans表示最优解 cnt表示算法得到的解

​		cnt>=ans  cnt 覆盖了cnt个 没有交集的区间 而 ans 还覆盖了一些额外的区间 

```c++
  sort(p + 1, p + n + 1);
  int point = -INF, ans = 0;
  for (int i = 1; i <= n; i++) {
    if (point >= p[i].second && point <= p[i].first)
      continue;
    else
      point = p[i].first, ans++;
  }
...
证明 ans<=cnt  反证 ans>cnt  cnt 个点覆盖了所有的区间 但是 ans个点覆盖了没有交集的区间 而ans>cnt 这是不可能的

​		cnt<=ans  cnt 选出的一定没有交集 所以是符合条件的解 ans是最优解是最大值  

##### 区间分组

将所有区间按左端点从小到大枚举 O(nlogn)

从前往后处理每个区间 判断能否将其放到某个现有的组中 

证明 ans<=cnt ans是最优的区间分组数 cnt也是一个合法的分组数

​		ans>=cnt  证明 ans>cnt-1  算法在进行到cnt-1时  第一次遇到需要新建组的情况  实际上就是这个组的左端点 和cnt-1个组的点都有交集了  所以cnt-1个组无法完成没有交集的组的划分
...
```c++
    ...
    {
        auto item = range[i];
        if (heap.empty() || heap.top() >= r.l) heap.push(item.r);
        else
        {
            heap.pop();
            heap.push(item.r);
...
证明 ans<=cnt  算法做出来的解cnt是合法的

​		ans>=cnt   反证 假设 ans<cnt  把ans中的区间和cnt中的替换 解依然是合法的  因为cnt找到的是右端点最大的 
...
考虑交换第i个位置上的牛和第i+1个位置上的牛 交换前后 值的影响 
...
```c++
  sort(a, a + n);//按照ddl时间 从小到大排序
  long long ans = 0;
  priority_queue<int, vector<int>, greater<int>> q;//q中存放着所有预计完成的任务 和最小的p
  for (int i = 0; i < n; i++) {
    if (a[i].d <= q.size()) {//截止日期超过了工作数
      if (q.top() < a[i].p)
        ans -= q.top(), ans += a[i].p, q.pop(), q.push(a[i].p);//p表示收益
    } else
...
2. $n≤10^2 => O(n^3) ,O(n^3logn)$，floyd，dp，高斯消元
3. $n≤10^3  => O(n^2),O(n^2logn)$，dp，二分+x，朴素版Dijkstra、朴素版Prim、Bellman-Ford
4. $n≤10^4 => O(n∗\sqrt{n})$，块状链表、分块、莫队
5. $n≤10^5 => O(nlogn)$各种sort，线段树、树状数组、set/map、heap、拓扑排序、dijkstra+heap、prim+heap、Kruskal、spfa、求凸包、求半平面交、二分+x、CDQ分治、整体二分+x、后缀数组、树链剖分、动态树
